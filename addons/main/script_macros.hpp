...
#define DB_COMMON(key,args) \
    ...
    GVAR(inuse) = true; \
    _response = "ArmaDragonflyClient" callExtension [_operation, _args]; \
    _result = _response select 0; \
    switch (_result) do { \
        case "NotFound"; \
        case "[]": { \
            diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_%1' Can't find Key '%2'", _operation, _key]; \
        }; \
        case "OK": { \
            diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_%1' Key '%2' is being sent in chunks to external function", _operation, _key]; \
        }; \
        default { \
            _data = (call compile _result); \
            diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_%1' Data '%2'", _operation, _data]; \
        }; \
    }; \
    GVAR(inuse) = false; \
    if (_obj isNotEqualTo "") then { \
