...
if (_key isEqualTo "" || _key isEqualTo []) exitWith {
    diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_del' Invalid Input for Key(s) '%1'", _key];
};

waitUntil { !GVAR(inuse) };
GVAR(inuse) = true;

private _result = "ArmaDragonflyClient" callExtension ["del", [_key]];
