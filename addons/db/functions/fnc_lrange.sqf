 ...
 *
 * Example:
 * ["myKey", 1, 10] call dragonfly_db_fnc_lrange; (Server or Singleplayer only)
 * ["myKey", 1, 10, myObject, "myFunction"] remoteExec ["dragonfly_db_fnc_lrange", 2, false]; (Multiplayer only)
 *
 * Public: Yes
 */
...
if (_key == "") exitWith {
    diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_lrange' Invalid Input for Key '%1'", _key];
};
