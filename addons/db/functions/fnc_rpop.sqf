 ...
 *
 * Example:
 * ["myKey", 1] call dragonfly_db_fnc_rpop; (Server or Singleplayer only)
 * ["myKey", 1, myObject, "myFunction"] remoteExec ["dragonfly_db_fnc_rpop", 2, false]; (Multiplayer only)
 *
 * Public: Yes
 */
...
if (_key == "") exitWith {
    diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_rpop' Invalid Input for Key '%1'", _key];
};
