 ...
 *
 * Example:
 * ["myKey", 1] call dragonfly_db_fnc_lpop; (Server or Singleplayer only)
 * ["myKey", 1, myObject, "myFunction"] remoteExec ["dragonfly_db_fnc_lpop", 2, false]; (Multiplayer only)
 *
 * Public: Yes
 */
...
if (_key == "") exitWith {
    diag_log text format ["ArmaDragonflyClient: 'dragonfly_db_fnc_lpop' Invalid Input for Key '%1'", _key];
};
