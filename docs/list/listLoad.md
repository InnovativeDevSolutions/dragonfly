...
```sqf
[_key, _start, _end, _object, _function, _call] call dragonfly_db_fnc_lrange
```
...
```sqf
["events", 0, -1, "", "myProject_fnc_processEventLogs"] call dragonfly_db_fnc_lrange;
```

**Retrieve a player list with synchronous callback:**
```sqf
["playerList", 0, -1, "", "myProject_fnc_processPlayerList", true] call dragonfly_db_fnc_lrange;
```

**Send mission data to a specific client:**
```sqf
["missionData", 0, -1, player, "myProject_fnc_processMissionData"] remoteExecCall ["dragonfly_db_fnc_lrange", 2, false];
```
...
## Related Functions
- `dragonfly_db_fnc_lpush`: Adds an element to the front of a list
- `dragonfly_db_fnc_rpush`: Adds an element to the back of a list
