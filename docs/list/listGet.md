---
title: ArmaDragonflyClient - List Get
icon: mdi:file-text-outline
excerpt: Get element of list stored at key from RAMDb.
---

# dragonfly_db_fnc_lindex
...
## Syntax
```sqf
[_key, _index, _object, _function, _call] call dragonfly_db_fnc_lindex
```
...
```sqf
["events", 0] call dragonfly_db_fnc_lindex;
```

### Retrieve data with synchronous callback:
```sqf
["messages", 5, "", "dragonfly_db_fnc_processMessage", true] call dragonfly_db_fnc_lindex;
```

### Retrieve data and send it to a specific client:
```sqf
["notifications", 0, player, "dragonfly_db_fnc_displayNotification"] remoteExecCall ["dragonfly_db_fnc_lindex", 2, false];
```
