...
## Syntax
```sqf
[_key, _data] call dragonfly_db_fnc_hmset
```
...
```sqf
["", ["loadout", [getUnitLoadout player], "position", [getPosASLVisual player]]] call dragonfly_db_fnc_hmset;
[getPlayerUID player, ["loadout", [getUnitLoadout player], "position", [getPosASLVisual player]]] call dragonfly_db_fnc_hmset;
```

### Store multiple player settings:
```sqf
["mykey", ["difficulty", ["regular"], "respawn", [true], "tickets", [500]]] call dragonfly_db_fnc_hmset;
```

### Store player data from a client:
```sqf
["", ["name", [name player], "uid", [getPlayerUID player], "score", [score player]]] remoteExecCall ["dragonfly_db_fnc_hmset", 2, false];
```
...
## Related Functions
- `dragonfly_db_fnc_hget`: Retrieves a field value from the current/specific client's hash table
- `dragonfly_db_fnc_hgetall`: Retrieves all fields from the current/specific client's hash table
- `dragonfly_db_fnc_hrem`: Removes a field from the current/specific client's hash table
- `dragonfly_db_fnc_hset`: Sets a field value pair in the current/specific client's hash table
