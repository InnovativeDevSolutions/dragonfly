...
## Syntax
```sqf
[_key, _keyField, _object, _function, _call] call dragonfly_db_fnc_hget
```
...
```sqf
["", "loadout"] call dragonfly_db_fnc_hget;
[getPlayerUID player, "loadout"] call dragonfly_db_fnc_hget;
```

### Retrieve data with synchronous callback:
```sqf
["", "playerScore", "", "dragonfly_db_fnc_processScore", true] call dragonfly_db_fnc_hget;
```

### Retrieve data and send it to a specific client:
```sqf
[getPlayerUID player, "loadout", netId player, "dragonfly_db_fnc_test", false] remoteExecCall ["dragonfly_db_fnc_hget", 2, false];
```
...
- `dragonfly_db_fnc_hmset`: Sets multiple field value pairs in the current/specific client's hash table
