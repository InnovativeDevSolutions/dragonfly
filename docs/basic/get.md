...
## Syntax
```sqf
[_key, _object, _function, _call] call dragonfly_db_fnc_get
```
...
```sqf
[getPlayerUID player, netId player, "dragonfly_db_fnc_test"] remoteExecCall ["dragonfly_db_fnc_get", 2, false];
```
...
- `dragonfly_db_fnc_delete`: Removes a key
